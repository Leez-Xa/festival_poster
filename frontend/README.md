...
页面右上角可以改 API 地址。若服务器设置了 `API_ACCESS_TOKEN`，在“访问令牌”输入框填写同一个值；该令牌只保存在浏览器 `localStorage`，不会写入项目文件。前端只调用 PRD 12.13 定义的后端接口，不直接调用 GPT、image2.0 或任何中转站接口，也不保存 AI API Key。
...
- 任务结果中的 `composition_json_url` 当前指向受保护 API：`/api/v1/poster-tasks/{task_id}/composition`。
